...
docker build -t openrevenue-platform -f apps/platform/Dockerfile .
...
# Run the background jobs container (uses the same image)
docker run -d \
  --name openrevenue-jobs \
  --env-file .env \
  -e APP_MODE=jobs \
...
```bash
docker compose -f docker-compose.prod.yml up -d platform jobs
```
