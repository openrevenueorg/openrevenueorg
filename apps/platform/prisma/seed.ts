...
async function main() {
  ...
  const categories = await Promise.all([
    ...
    prisma.category.upsert({
      ...
      create: {
        name: 'A/C Repair',
        slug: 'ac-repair',
        description: 'A/C repair and maintenance services',
        icon: '🎨',
      },
    }),
    prisma.category.upsert({
  ...
  if (process.env.NODE_ENV === 'development') {
    ...
    const demoStartupAdmin = await prisma.startup.upsert({
    ...
    await prisma.privacySettings.upsert({
      where: { startupId: demoStartupAdmin.id },
      update: {},
      create: {
        startupId: demoStartupAdmin.id,
        showRevenue: 'public',
        showMRR: 'public',
        showARR: 'public',
        showCustomerCount: 'public',
