...
type FeaturedStartupCandidate = Prisma.StartupGetPayload<{
  include: {
    category: true;
    connections: {
      select: {
...
export async function GET(req: NextRequest) {
  try {
    ...
    const allStartups: FeaturedStartupCandidate[] = await prisma.startup.findMany({
    ...
    const selected: FeaturedStartupCandidate[] = [];
...
function selectDiverse(
  startups: FeaturedStartupCandidate[],
  count: number
): FeaturedStartupCandidate[] {
  if (count <= 0) return [];
