...
async function getFounder(username: string) {
  try {
...
export default async function FounderPage({ params }: { params: { username: string } }) {
  ...
  // Calculate stats
  const startupsWithRevenue = founder.startups.filter((s) => s.revenueSnapshots.length > 0);
  const totalRevenue = founder.startups.reduce((sum, startup) => {
    const latest = startup.revenueSnapshots[0];
    if (latest) {
      return sum + (latest.revenue || 0);
  ...
  const last30DaysRevenue = founder.startups.reduce((sum, startup) => {
    const recent = startup.revenueSnapshots.filter((rs) => {
      const date = new Date(rs.date);
      return date >= thirtyDaysAgo;
    });
    return sum + recent.reduce((s, r) => s + r.revenue, 0);
  }, 0);
  ...
  const allDates = founder.startups
    .flatMap((s) => s.revenueSnapshots.map((rs) => new Date(rs.date)))
    .sort((a, b) => b.getTime() - a.getTime());
  const recentRevenueDate = allDates[0];
