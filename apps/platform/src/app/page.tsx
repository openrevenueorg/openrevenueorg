...
async function getFeaturedStartups(): Promise<FeaturedStartupSummary[]> {
  try {
    const limit = 6;

    // Get all published startups with recent revenue data
    const allStartups: HomepageStartupCandidate[] = await prisma.startup.findMany({
    ...
    const selected: HomepageStartupCandidate[] = [];
    ...
    if (selfReported.length > 0) {
      const selfReportedSelected = selectDiverse(selfReported, selfReportedCount);
    ...
    const remaining = startupsWithRevenue.filter(
      (startup) => !selected.find((selectedStartup) => selectedStartup.id === startup.id)
    );
    ...
    // Format response
    return shuffled.map<FeaturedStartupSummary>((startup) => {
      const latestSnapshot = startup.revenueSnapshots[0];
      const trustLevel =
        startup.connections[0]?.trustLevel === 'PLATFORM_VERIFIED'
          ? 'PLATFORM_VERIFIED'
          : 'SELF_REPORTED';
...
function selectDiverse(
  startups: HomepageStartupCandidate[],
  count: number
): HomepageStartupCandidate[] {
  if (count <= 0) return [];
  ...
  const p33 = revenues[Math.floor(revenues.length * 0.33)] ?? 0;
  const p66 = revenues[Math.floor(revenues.length * 0.66)] ?? 0;
...
export default async function HomePage() {
  ...
  const structuredData = {
    ...
    sameAs: [
      'https://github.com/openrevenueorg/openrevenueorg',
    ],
    contactPoint: {
      '@type': 'ContactPoint',
                      ...
                      <div className="space-y-4">
                        ...
                        <div>
                          <TrustBadge
                            ...
                            verificationMethod={startup.verificationMethod ?? undefined}
                            size="sm"
                          />
                        </div>
            ...
            <div className="max-w-3xl mx-auto">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                ...
                <Button
                  size="lg"
                  variant="outline"
                  className="text-lg px-8 border-primary-foreground/20 text-primary-foreground bg-primary hover:bg-primary-foreground/20"
                  asChild
                >
